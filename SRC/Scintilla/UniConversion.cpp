...
// converts a UCS-2 string to a UTF-8 string. call with outputBufferSize=0 to obtain length
unsigned UTF8FromUCS2(const wchar_t *inputBuffer, unsigned int inputLen, char *outputBuffer, unsigned int outputBufferSize, int fBigEndian) {
	if (!outputBufferSize) {
		if (!fBigEndian) {
			const wchar_t *uend = inputBuffer + inputLen;
			unsigned int len = 0;
			for (; inputBuffer < uend; inputBuffer++) {
				len++;
				if (*inputBuffer >= 0x0080)
					len++;
				if (*inputBuffer >= 0x0800)
					len++;
			}
			return len;
		} else {
			...
			for (; inputBuffer < uend; inputBuffer++) {
				len++;
				if (*inputBuffer & 0x80FF)
					len++;
				if (*inputBuffer & 0x00F8)
					len++;
			}
			return len;
		}
	} else if (!fBigEndian) {
		const wchar_t *uptr = inputBuffer, *uend = inputBuffer + inputLen;
		unsigned char *uputf = (unsigned char *)outputBuffer, *upend = uputf + outputBufferSize;
		...
		return((char *)uputf - outputBuffer);
	} else {
		const wchar_t *uptr = inputBuffer, *uend = inputBuffer + inputLen;
		unsigned char *uputf = (unsigned char *)outputBuffer, *upend = uputf + outputBufferSize;
		for (; uptr < uend && uputf < upend; uptr++) {
			...
			if (!(uch & 0x80FF)) { // 0nnnnnnn00000000 -> nnnnnnnn
				...
				*uputf++ = (unsigned char)(0x80 | ((uch >> 8) & 0x3f));
				...
				if (uputf >= upend - 2)
					break; // 1110qqqq 10xxxxvv 10nnnnnn
...
// converts a UTF-8 string to a UCS-2 string. call with inputLen=0 for length
		...
		while (sx < endx && tbuf < endt) {
			if (*sx < 0x80) {
				*tbuf++ = *sx;
				...
				if (sx >= endx - 1)
					break;                 // 00000xxxvvnnnnnn
				*tbuf++ = ((*sx & 0x1F) << 6) | (sx[1] & 0x3F);
				sx += 2;
			} else {                 // 1110qqqq 10xxxxvv 10nnnnnn
				if (sx >= endx - 2)
					break; // qqqqxxxxvvnnnnnn
		...
		while (sx < endx && tbuf < endt) {
			if (*sx < 0x80) {
				*tbuf++ = (*sx) << 8;
				...
				if (sx >= endx - 1)
					break;                 // vvnnnnnn00000xxx
				*tbuf++ = ((*sx & 0x3) << 14) | ((*sx & 0x1C) >> 2) | ((sx[1] & 0x3F) << 8);
				sx += 2;
			} else {                 // 1110qqqq 10xxxxvv 10nnnnnn
				if (sx >= endx - 2)
					break; // vvnnnnnnqqqqxxxx
